...
function curl_get($url, $args){
	...
	function jblog_publish($post_ID, $output = OBJECT){

	     global $wpdb;
	     $post = $wpdb->get_row( $wpdb->prepare( "SELECT post_content FROM $wpdb->posts WHERE ID = %s AND post_type='post'", $post_ID), $output);  
            $article = nl2br($post->post_content);
	...
	if(!has_action('publish_post','jblog_publish'))add_action('publish_post','jblog_publish');
